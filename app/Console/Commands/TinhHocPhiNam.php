...
class TinhHocPhiNam extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:tinh-hoc-phi-nam';
    ...
    {
        $hoc_sinhs = HocSinhModel::where('trang_thai', 1)->where('loai_hoc_phi',1)->get();
        foreach ($hoc_sinhs as $hoc_sinh){
            $hoc_phi = new HocPhiModel();
            $hoc_phi->id_hoc_sinh = $hoc_sinh->id;
            $hoc_phi->loai_hoc_phi = 1;
            $tong_dich_vu = 0;
            $tien_nang_khieu = 0;
            ...
            $tong_hoc_phi = BangGiaModel::where('ten_gia','Học phí hệ '.$he_dao_tao->ten_he_dao_tao.'')->first()->gia*12;
